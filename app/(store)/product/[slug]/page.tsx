...
import { ShieldCheck, Star, Truck, WalletCards } from "lucide-react";
...
export default async function ProductPage({ params }: { params: Params }) {
  ...
  const related = (await getPublishedProducts({ category: product.category })).filter((item) => item.id !== product.id).slice(0, 4);
  const uniqueImages = product.images.filter((src, index, list) => list.indexOf(src) === index);
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_.85fr]">
        <section className="grid gap-4 sm:grid-cols-[90px_1fr]">
          <div className="hidden gap-3 sm:grid">
            {uniqueImages.map((src, index) => (
              <div key={`${product.id}-${src}-${index}`} className="relative aspect-square overflow-hidden rounded-md border border-rosewood/10 bg-white">
          ...
          <div className="relative aspect-[3/4] overflow-hidden rounded-md bg-white shadow-soft">
            <ProductImage src={uniqueImages[0] ?? product.images[0]} alt={product.name} fill className="object-cover transition duration-500 hover:scale-105" priority />
        ...
        <section>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {[[Truck, product.deliveryEstimate], [WalletCards, "COD, bKash, card"], [ShieldCheck, "Easy exchange policy"]].map(([Icon, text]) => (
